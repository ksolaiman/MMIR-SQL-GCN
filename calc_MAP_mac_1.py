...
# NOTE: This function was originally added as an experimental alternative using scikit-learn's average_precision_score.
# It is NOT suitable for retrieval ranking MAP evaluation in this project.
# average_precision_score treats distances as uncalibrated binary classification scores and thresholds them globally,
# ignoring per-query ranking. This loses the core ranking-based nature of retrieval MAP, where results must be
# sorted for each query and precision measured at each rank. For correct retrieval evaluation, use the manual
# ranking-based fx_calc_map_label_detailed() instead.
...
def fx_calc_map_label_detailed(db_items, query_items, dist, ground_dist, item2idx, k):
    ...
    def calMAP(_k):     # @k??
        ...
        for i, mapped_i in enumerate(query_indices):
            ...
            if r > 0:
                _res += [p / relAll]              # AP = RunningSum / Total#ofRelevantDocumentsInGT                     # previously was dividing by r, and was ok, bcoz the cond. was only 2nd one when gt was relevant, but now we have 2 conds. if prediction is relevant and gt is relevant
            else:
                _res += [0]
        ...
        for query_no in non_zero_indices:
            ...
            for recallLevel in np.arange(0, 1.1, 0.1):  # 0.001 with 0.001 step until 1: 1000 iterations
                ...
                # Official IR formulation: For each standardized recall level r, find the maximum precision achieved at any recall ≥ r. 
                # So in new calc_MAP function i changed to v >= recallLevel
...
# ####################
# # with open("latest-simgnn/data/testset/texttestset.pkl", "wb") as f:
# #     pickle.dump(texttestset, f)
# # with open("latest-simgnn/data/testset/imagetestset.pkl", "wb") as f:
# #     pickle.dump(imagetestset, f)
# # with open("latest-simgnn/data/testset/videotestset.pkl", "wb") as f:
# #     pickle.dump(videotestset, f)
# #############
# # with open("latest-simgnn/data/testset/texttestset.pkl", "wb") as f:
# #     pickle.dump(texttestset, f)
# # with open("latest-simgnn/data/testset/texttestset.pkl", "wb") as f:
...
# # # #
# maps = []
# maps.append(fx_calc_map_label_detailed(texttestset, texttestset, dist, ground_dist, k = 0))         # !!
# maps.append(fx_calc_map_label_detailed(videotestset, texttestset, dist, ground_dist, k = 0))
# maps.append(fx_calc_map_label_detailed(imagetestset, texttestset, dist, ground_dist, k = 0))
# maps.append(fx_calc_map_label_detailed(testset, texttestset, dist, ground_dist, k = 0))
# #
# maps.append(fx_calc_map_label_detailed(videotestset, videotestset, dist, ground_dist, k = 0))
# maps.append(fx_calc_map_label_detailed(texttestset, videotestset, dist, ground_dist, k = 0)) #
# maps.append(fx_calc_map_label_detailed(imagetestset, videotestset, dist, ground_dist, k = 0))
# maps.append(fx_calc_map_label_detailed(testset, videotestset, dist, ground_dist, k = 0))
# #
# maps.append(fx_calc_map_label_detailed(imagetestset, imagetestset, dist, ground_dist, k = 0))
# maps.append(fx_calc_map_label_detailed(texttestset, imagetestset, dist, ground_dist, k = 0))  #
# maps.append(fx_calc_map_label_detailed(videotestset, imagetestset, dist, ground_dist, k = 0))   #
# maps.append(fx_calc_map_label_detailed(testset, imagetestset, dist, ground_dist, k = 0))
